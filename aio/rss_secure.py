"""
nyt_ticker.py — NYT Politics RSS ticker (Windows 11 Fixed) - SECURE VERSION
─────────────────────────────────────────────────────────────────────────────
This is a minimally modified version of the original rss.py that fixes the
critical SSL security vulnerability while keeping everything else the same.
"""
...
def fetch_loop(update_queue):
    ...
    while True:
        try:
            ...
            # Create SSL context - SECURITY FIX: Try proper verification first
            ssl_context = ssl.create_default_context()
            
            if ssl_verify_failed:
                # Only disable verification if it failed before (corporate proxy issues)
                debug_print("WARNING: SSL verification disabled due to previous failures")
            ...
            try:
                ...
                if "certificate verify failed" in str(e).lower() and not ssl_verify_failed:
                    debug_print("SSL certificate verification failed. Retrying without verification...")
                    ssl_verify_failed = True
                    continue  # Retry the loop with verification disabled
        ...
        try:
            ...
            if clicked_item and clicked_item['url']:
                ...
                if url.startswith(('http://', 'https://')):
                    ...
                    debug_print(f"Refusing to open non-HTTP URL: {url}")
            else:
...
if __name__ == "__main__":
    debug_print("Starting NYT Politics RSS ticker (SECURE VERSION)")
